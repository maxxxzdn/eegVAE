...
class Encoder_x(nn.Module):
    def __init__(self, n_nodes, drop_rate = 0.1):
        super().__init__()
...
class Encoder_z(nn.Module):
    def __init__(self, latent_dim, n_nodes, drop_rate, aggr_z):
        super().__init__()
        self.latent_dim = latent_dim
        self.fc1 = nn.Sequential(
            Aggregator(aggr_z),
            nn.Linear(latent_dim, 400) if aggr_z != 'none' else nn.Linear(n_nodes*latent_dim, 400),
            nn.Dropout(drop_rate),
            nn.ReLU())
...
class VAE(nn.Module):
    def __init__(self, args):
        ...
        self.encoder_x = Encoder_x(args.n_nodes, args.drop_rate)
        self.decoder_z = Decoder_z(args.latent_dim_z, args.n_nodes, args.drop_rate)
        self.encoder_z = Encoder_z(args.latent_dim_z, args.n_nodes, args.drop_rate, args.aggr_z)
        self.decoder_w = Decoder_w(args.latent_dim_w, args.n_nodes, args.drop_rate, args.decoder_w_mode)
        
        self.loc_z = nn.Linear(48, args.latent_dim_z)
        self.logscale_z = nn.Linear(48, args.latent_dim_z)
    ...
    def forward(self, data):
        #x_encoded
        x_encoded = self.encoder_x(data.x)
        #μ(x)
